...
class Class_Ejemplo(APIView):
    ...
    def get(self, request):
        ...
        # return JsonResponse({"estado":"ok", "mensaje":f"Método GET | id={request.GET.get('id', None)} | slug={request.GET.get('slug')}"}, status=200)
    ...
    def post(self, request):
        ...
        # return JsonResponse({"estado":"ok", "mensaje":f"Método POST | correo={request.data.get('correo')} | password={request.data.get('password')}"}, status=201)
...
class Class_EjemploParametros(APIView):
    ...
    def get(self, request, id):
        ...
        return JsonResponse({"estado":"ok", "mensaje":f"Método GET | parámetro={id}"}, status=HTTPStatus.OK)
    
    
    def put(self, request, id):
        # return HttpResponse(f"Método PUT | parámetro={id}")
        return JsonResponse({"estado":"ok", "mensaje":f"Método PUT | parámetro={id}"}, status=HTTPStatus.OK)
    
    
    def delete(self, request, id):
        # return HttpResponse(f"Método DELETE | parámetro={id}")
        return JsonResponse({"estado":"ok", "mensaje":f"Método DELETE | parámetro={id}"}, status=HTTPStatus.OK)
